...
public partial class MainController : ApplicationContext {
    ...
    private async Task UpdateTray(HashSet<Lighthouse> lighthouses)
    {
        ...
        var lighthouseItems = lighthouses.Select(l => new ToolStripMenuItem(l.Name) { Enabled = false });
        ...
        var scanNowItem = new ToolStripMenuItem("Scan Now", null, (_, _) =>
        {
            _btWatcher.StartScan();
        });
